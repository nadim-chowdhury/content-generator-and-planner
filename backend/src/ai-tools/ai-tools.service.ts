...
export class AiToolsService {
  ...
  async generateScript(dto: ScriptGeneratorDto) {
    ...
    const scriptType =
      dto.type === ScriptType.SHORT ? 'short-form' : 'long-form';
    const duration =
      dto.targetDuration || (dto.type === ScriptType.SHORT ? 30 : 600);
        ...
        messages: [
          {
            role: 'system',
            content:
              'I need help rewriting some content to make it sound more natural and engaging. Write it like a real person would, not like a corporate robot. Keep it authentic and conversational.',
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
...
- estimatedDuration: Estimated duration in seconds (if applicable)`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              "I'm brainstorming content ideas and need help expanding on this one. Think like a real creator would - what would make this idea actually interesting and relatable? Make it feel authentic.",
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
      ...
      console.error('OpenAI API Error:', error);
      throw new BadRequestException(
        'Failed to elaborate idea. Please try again.',
      );
    }
  }
...
- recommendations: Best practices for this platform/niche`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'I need help making this title better. Give me variations that sound natural and click-worthy, not clickbait. Think about what would make a real person want to click, not what an algorithm wants.',
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
      ...
      console.error('OpenAI API Error:', error);
      throw new BadRequestException(
        'Failed to optimize title. Please try again.',
      );
    }
  }
        ...
        messages: [
          {
            role: 'system',
            content:
              "I'm planning my content calendar and need ideas that feel authentic and varied. Don't make everything sound the same - mix it up like a real creator would. Make it feel natural.",
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
        ...
        messages: [
          {
            role: 'system',
            content:
              "I want to understand what my competitors are doing, but analyze it like a real person would - what's actually working, what feels authentic, what's just noise. Give me insights that matter.",
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
...
- monetization: Monetization opportunities`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              "I'm researching this niche and need real insights, not generic market research. Tell me what's actually happening, what real creators are doing, and what opportunities exist. Be specific and authentic.",
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
      ...
      console.error('OpenAI API Error:', error);
      throw new BadRequestException(
        'Failed to research niche. Please try again.',
      );
    }
  }
...
- overallTrends: Overall trend patterns in this niche`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              "I want to know what's actually trending right now, not what some algorithm says. Give me real trends that real people are talking about. Make it feel current and authentic.",
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
      ...
      console.error('OpenAI API Error:', error);
      throw new BadRequestException(
        'Failed to generate trending topics. Please try again.',
      );
    }
  }
...
- messaging: Recommended messaging approach`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'I need to understand my audience better. Create a persona that feels like a real person, not a marketing stereotype. What would this person actually care about? How do they really behave? Be authentic.',
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
      ...
      console.error('OpenAI API Error:', error);
      throw new BadRequestException(
        'Failed to build audience persona. Please try again.',
      );
    }
  }
...
- engagementPotential: Estimated engagement potential (0-100)`;
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              "I want an honest assessment of whether this content could go viral. Be realistic - not everything goes viral. Tell me what would actually work and what wouldn't, like a real creator would analyze it.",
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
      ...
      console.error('OpenAI API Error:', error);
      throw new BadRequestException(
        'Failed to predict viral score. Please try again.',
      );
    }
  }
}
