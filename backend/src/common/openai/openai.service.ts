...
export class OpenAIService implements OnModuleInit {
  ...
  private initializeApiKeys() {
    ...
    if (additionalKeys) {
      ...
      const keys = additionalKeys.split(/[,\s]+/).filter(key => key.trim().length > 0);
      allKeys.push(...keys);
    }
  ...
  private markKeyError(keyStatus: ApiKeyStatus, error: any) {
    ...
    if (keyStatus.errorCount >= this.maxErrorsPerKey) {
      ...
      this.logger.warn(`API key deactivated due to errors. Will retry after ${this.errorResetTime}ms`);
    ...
    if (error?.status === 429) {
      ...
      if (retryAfter) {
        keyStatus.rateLimitResetAt = new Date(Date.now() + parseInt(retryAfter) * 1000);
        keyStatus.isActive = false;
  ...
  async createChatCompletion(
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      ...
      // Check rate limit
      if (keyStatus.rateLimitResetAt && new Date() < keyStatus.rateLimitResetAt) {
    ...
    throw lastError || new Error('Failed to complete OpenAI request after retries');
  }
  ...
  private humanizeSystemPrompt(prompt: string): string {
    ...
    humanized += ' Write naturally, like a real person would, with personality and authenticity. Avoid overly formal or robotic language.';
  ...
  getHumanLikePrompt(context: {
    ...
    const { task, tone = 'conversational', style = 'engaging', platform, niche, language } = context;
