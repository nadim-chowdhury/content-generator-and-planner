...
export default function IdeasPage() {
  ...
  // Viral score filters
  const [viralScoreMin, setViralScoreMin] = useState<number | undefined>(
    undefined
  );
  const [viralScoreMax, setViralScoreMax] = useState<number | undefined>(
    undefined
  );
  ...
  const handleBulkOperation = async (
    ...
    if (
      operation === "DELETE" &&
      !confirm(`Are you sure you want to delete ${selectedIdeas.size} idea(s)?`)
    ) {
      return;
    }
    ...
    try {
      if (operation === "EXPORT") {
        const selectedIdeasList = ideas.filter((idea) =>
      ...
      await loadIdeas();
      alert(
        `${selectedIdeas.size} idea(s) ${operation.toLowerCase()}d successfully`
      );
    } catch (err: any) {
      alert(
        err.response?.data?.message ||
          `Failed to ${operation.toLowerCase()} ideas`
      );
    }
  };
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Facebook Reels">
                        Facebook Reels
                      </SelectItem>
                ...
                <div className="mb-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={
                        selectedIdeas.size === ideas.length && ideas.length > 0
                      }
              ...
              <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Array.from(
                      new Set(
                        ideas.map((idea) => idea.language).filter(Boolean)
                      )
                    ).map((langCode) => {
                      const count = ideas.filter(
                        (idea) => idea.language === langCode
                      ).length;
            ...
            {ideas.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-gray-600 dark:text-gray-400">
                  No ideas yet. Generate some from the dashboard!
                </p>
              </div>
            ) : (
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              viralScore: parseInt(e.target.value) || undefined,
                            })
                          }
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            thumbnailSuggestion: e.target.value,
                          })
                        }
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        rows={2}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            platformOptimization: e.target.value,
                          })
                        }
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        rows={2}
              ...
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Export {ideas.length} idea{ideas.length !== 1 ? "s" : ""} in
