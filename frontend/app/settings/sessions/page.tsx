...
export default function SessionsPage() {
  ...
  const handleRevokeSession = async (sessionId: string) => {
    ...
    try {
      ...
      const currentSession = sessions.find(s => s.isCurrent && s.id === sessionId);
      if (currentSession) {
        clearAuth();
        router.push('/login');
      }
    ...
    <ProtectedRoute>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
          <div>
            <h2 className="font-semibold">Your Sessions ({sessions.length})</h2>
            <p className="text-sm text-gray-600">All active sessions across your devices</p>
          </div>
          {sessions.length > 1 && (
            <button
            ...
            sessions.map((session) => (
              <div
                ...
                className={`p-4 hover:bg-gray-50 transition-colors ${
                  session.isCurrent ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                }`}
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      <div>
                        <span className="font-medium">Last Active:</span> {formatDate(session.lastUsedAt)}
                      </div>
                      <div>
      ...
      <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
        ...
        <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
          ...
          <li>Use strong, unique passwords and enable 2FA for better security</li>
        </ul>
      </div>
        </div>
      </div>
