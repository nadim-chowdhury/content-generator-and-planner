...
export default function BillingPage() {
  ...
  const [recentBillingNotifications, setRecentBillingNotifications] = useState<
    any[]
  >([]);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [statusData, invoicesData, usageData, notificationsData] =
        await Promise.all([
      ...
      if (notificationsData && Array.isArray(notificationsData)) {
        const billingNotifs = notificationsData.filter(
          (n: any) =>
            n.category === "SYSTEM" &&
                  ...
                  <CardContent>
                    ...
                    {!usage.isUnlimited && (
                      <div className="space-y-2">
                        ...
                        <Progress
                          value={
                            (usage.dailyGenerations / (usage.dailyLimit || 1)) *
                            100
                          }
                        />
                      </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setCancelAtPeriodEnd(checked as boolean)
                    }
                  />
