...
export const adminAnalyticsApi = {
  getBusinessAnalytics: async (
    ...
    const { data } = await api.get<BusinessAnalyticsSummary>(
  ...
  getDailySignups: async (days?: number): Promise<DailySignups[]> => {
    const params: any = {};
    if (days) params.days = days;
    const { data } = await api.get<DailySignups[]>(
      "/api/admin/analytics/daily-signups",
      { params }
    );
    return data;
  },
  ...
  getChurnRate: async (days?: number): Promise<number> => {
    const params: any = {};
    if (days) params.days = days;
    const { data } = await api.get<number>("/api/admin/analytics/churn-rate", {
      params,
    });
    return data;
  },
  ...
  getTopNiches: async (limit?: number): Promise<TopNiche[]> => {
    ...
    const { data } = await api.get<TopNiche[]>(
      "/api/admin/analytics/top-niches",
      { params }
    );
    return data;
  },
