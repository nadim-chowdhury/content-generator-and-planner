...
export function exportToCSV(ideas: Idea[]): void {
  ...
  link.setAttribute(
    "download",
    `content-ideas-${new Date().toISOString().split("T")[0]}.csv`
  );
...
`;
  ...
  link.setAttribute(
    "download",
    `content-ideas-${new Date().toISOString().split("T")[0]}.txt`
  );
  link.style.visibility = "hidden";
  document.body.appendChild(link);
  link.click();
...
export async function exportToGoogleSheets(
  ...
  try {
    if (!spreadsheetId) {
      const input = prompt(
        "Enter Google Sheets Spreadsheet ID (or leave empty to get CSV data):"
      );
      if (!input) {
    ...
    if (response.data.csvData) {
      ...
      const blob = new Blob([response.data.csvData], {
      ...
      alert(
        "CSV file downloaded. Import it into Google Sheets manually, or configure Google Sheets API integration for direct export."
      );
    }
  } catch (error) {
    console.error("Failed to export to Google Sheets:", error);
    alert(
      "Failed to export to Google Sheets. Please try again or use CSV export instead."
    );
  }
}
...
export async function exportToNotion(
  ...
  try {
    if (!databaseId) {
      const input = prompt(
        "Enter Notion Database ID (or leave empty to get JSON data):"
      );
      if (!input) {
    ...
    if (response.data.data) {
      ...
      alert(
        "JSON file downloaded. Import it into Notion manually, or configure Notion API integration for direct export."
      );
    }
  } catch (error) {
    console.error("Failed to export to Notion:", error);
    alert(
      "Failed to export to Notion. Please try again or use JSON export instead."
    );
  }
}
