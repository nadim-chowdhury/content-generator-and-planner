...
export const infrastructureApi = {
  ...
  getIPStatistics: async (ip: string): Promise<IPStatistics> => {
    const { data } = await api.get<IPStatistics>(`/api/infrastructure/ddos/ip/${ip}`);
    return data;
  },

  blockIP: async (ip: string, reason?: string): Promise<{ message: string }> => {
    const { data } = await api.post<{ message: string }>(`/api/infrastructure/ddos/block/${ip}`, { reason });
    return data;
  },

  unblockIP: async (ip: string): Promise<{ message: string }> => {
    const { data } = await api.post<{ message: string }>(`/api/infrastructure/ddos/unblock/${ip}`);
    return data;
  },

  // Backups
  createBackup: async (type?: 'daily' | 'weekly' | 'monthly' | 'manual'): Promise<{
    success: boolean;
    filename: string;
    path: string;
    size: number;
    ...
    const { data } = await api.post('/api/infrastructure/backups/create', { type: type || 'manual' });
    return data;
  },
  ...
  getBackupStats: async (): Promise<BackupStats> => {
    const { data } = await api.get<BackupStats>('/api/infrastructure/backups/stats');
    return data;
  },

  restoreBackup: async (filepath: string): Promise<{ success: boolean; message: string }> => {
    const { data } = await api.post('/api/infrastructure/backups/restore', { filepath });
    return data;
  },
  ...
  listBaseBackups: async (): Promise<BaseBackup[]> => {
    const { data } = await api.get<BaseBackup[]>('/api/infrastructure/pitr/base-backups');
    return data;
  },
