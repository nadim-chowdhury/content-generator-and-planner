...
export const billingApi = {
  ...
  activateLifetime: async (
    ...
    const { data } = await api.post<{ message: string; plan: string }>(
      "/api/billing/activate-lifetime",
      {
        licenseKey,
      }
    );
  ...
  upgradeDowngrade: async (
    planType: PlanType
  ): Promise<{ message: string; plan: string; subscriptionId: string }> => {
    const { data } = await api.post<{
      message: string;
      plan: string;
      subscriptionId: string;
    }>("/api/billing/upgrade-downgrade", { planType });
    return data;
  },

  validateCoupon: async (couponCode: string): Promise<CouponValidation> => {
    const { data } = await api.post<CouponValidation>(
