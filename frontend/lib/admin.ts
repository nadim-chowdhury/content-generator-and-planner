...
export const adminApi = {
  ...
  updateUserPlan: async (
    ...
    const { data } = await api.put<User>(`/api/admin/users/${userId}/plan`, {
      plan,
    });
    return data;
  },
  ...
  addBonusCredits: async (
    ...
    const { data } = await api.post<{ message: string }>(
      `/api/admin/users/${userId}/bonus-credits`,
      { credits }
    );
    return data;
  },
  ...
  getUserQuota: async (userId: string): Promise<UserQuota> => {
    const { data } = await api.get<UserQuota>(
  ...
  getAllSubscriptions: async (
    ...
    const { data } = await api.get<{
      subscriptions: SubscriptionWithUser[];
      pagination: { total: number; page: number; limit: number; pages: number };
    }>(`/api/admin/billing/subscriptions?page=${page}&limit=${limit}`);
    return data;
  },
  ...
  getAllInvoices: async (
    ...
    const { data } = await api.get<{
      ...
      hasMore: boolean;
      pagination: { page: number; limit: number };
    }>(`/api/admin/billing/invoices?page=${page}&limit=${limit}`);
    return data;
  },
  ...
  getUserSubscription: async (
    ...
    const { data } = await api.get<SubscriptionWithUser>(
  ...
  getUserInvoices: async (
    userId: string,
    limit: number = 10
  ): Promise<{ user: { id: string; email: string }; invoices: Invoice[] }> => {
    const { data } = await api.get<{
      user: { id: string; email: string };
      invoices: Invoice[];
    }>(`/api/admin/billing/users/${userId}/invoices?limit=${limit}`);
    return data;
  },
  ...
  cancelUserSubscription: async (
    ...
    const { data } = await api.post<{ message: string }>(
      `/api/admin/billing/users/${userId}/cancel-subscription`,
      { immediately }
    );
    return data;
  },
  ...
  updateQuotaSettings: async (
    ...
    const { data } = await api.put(
      `/api/admin/settings/quotas/${plan}`,
      settings
    );
    return data;
  },
  ...
  updateStripeProductIds: async (settings: {
  ...
  // Content Moderation
  getFlaggedIdeas: async (
    page: number = 1,
    limit: number = 20
  ): Promise<any> => {
    const { data } = await api.get(
      `/api/admin/moderation/flagged?page=${page}&limit=${limit}`
    );
    return data;
  },
  ...
  flagIdea: async (
    ...
    const { data } = await api.post(`/api/admin/moderation/flag/${ideaId}`, {
      reason,
      category,
    });
    return data;
  ...
  reviewFlag: async (
    ...
    const { data } = await api.post(`/api/admin/moderation/review/${flagId}`, {
      action,
    });
    return data;
  },
  ...
  blockIdea: async (ideaId: string, reason: string): Promise<any> => {
    const { data } = await api.post(`/api/admin/moderation/block/${ideaId}`, {
      reason,
    });
    return data;
  },
  ...
  getBlacklistKeywords: async (
    page: number = 1,
    limit: number = 50
  ): Promise<any> => {
    const { data } = await api.get(
      `/api/admin/moderation/blacklist?page=${page}&limit=${limit}`
    );
    return data;
  },

  addBlacklistKeyword: async (
    keyword: string,
    category: string = "GENERAL",
    severity: string = "MEDIUM",
    action: string = "FLAG"
  ): Promise<any> => {
    const { data } = await api.post("/api/admin/moderation/blacklist", {
      keyword,
      category,
      severity,
      action,
  ...
  deleteBlacklistKeyword: async (keywordId: string): Promise<any> => {
    const { data } = await api.delete(
      `/api/admin/moderation/blacklist/${keywordId}`
    );
    return data;
  },

  updateBlacklistKeyword: async (
    keywordId: string,
    updates: {
      category?: string;
      severity?: string;
  ...
  getDailyActiveUsers: async (date?: string): Promise<{ count: number }> => {
    const { data } = await api.get(
      `/api/admin/analytics/dau${date ? `?date=${date}` : ""}`
    );
    return data;
  },
  ...
  getDailyActiveUsersTrend: async (
    ...
    const { data } = await api.get(
      `/api/admin/analytics/dau/trend?days=${days}`
    );
    return data;
  },
  ...
  getMonthlyActiveUsersTrend: async (
    months: number = 12
  ): Promise<Array<{ month: string; count: number }>> => {
    const { data } = await api.get(
      `/api/admin/analytics/mau/trend?months=${months}`
    );
    return data;
  },
  ...
  getSocialSharingMetrics: async (days: number = 30): Promise<any> => {
    const { data } = await api.get(
      `/api/admin/analytics/social-sharing?days=${days}`
    );
    return data;
  },
