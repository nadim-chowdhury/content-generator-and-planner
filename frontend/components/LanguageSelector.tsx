...
const popularLanguages = ['en', 'bn', 'hi', 'ar', 'es', 'fr', 'de', 'pt', 'ru', 'ja', 'ko', 'zh'];

export default function LanguageSelector({ value, onChange, showPopular = true }: LanguageSelectorProps) {
    ...
    <div className="relative">
      ...
      <div className="relative">
        ...
        >
          <div className="flex items-center gap-2">
            {selectedLang ? (
              <LanguageBadge languageCode={selectedLang.code} size="sm" showNativeName={true} />
            ) : (
          ...
          <>
            ...
            <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md shadow-lg max-h-96 overflow-auto">
              {showPopular && popularLangs.length > 0 && (
                <>
                  <div className="px-4 py-2 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Popular Languages</p>
                  </div>
                  {popularLangs.map((lang) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between ${
                        value === lang.code ? 'bg-indigo-50 dark:bg-indigo-900/20' : ''
                      }`}
                    >
              ...
              {otherLangs.length > 0 && (
                <>
                  <div className="px-4 py-2 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">More Languages</p>
                  </div>
                  {otherLangs.map((lang) => (
                    <button
                      key={lang.code}
      ...
      {selectedLang && selectedLang.rtl && (
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
          ⚠️ This language uses right-to-left (RTL) script. Content will be formatted accordingly.
        </p>
      )}
    </div>
  );
