...
const platformInfo: Record<string, {
  ...
  'YouTube': {
    ...
    bestPractices: ['detailed descriptions', 'timestamps/chapters', 'strong CTAs', 'high-quality visuals'],
  },
  'YouTube Shorts': {
    ...
    contentType: ['quick tips', 'challenges', 'viral trends', 'behind-the-scenes'],
    bestPractices: ['quick hooks', 'trending sounds', '#Shorts hashtag', 'vertical format'],
  },
  'TikTok': {
    optimalDuration: { min: 15, max: 60, recommended: 30 },
    aspectRatio: '9:16',
    ...
    hashtagCount: { min: 5, max: 15, recommended: 7 },
    contentType: ['quick tips', 'challenges', 'viral trends', 'behind-the-scenes'],
    bestPractices: ['quick hooks', 'trending sounds', 'trending hashtags', 'vertical format'],
  },
  'Instagram Reels': {
    ...
    bestPractices: ['trending audio', 'visual descriptions', '5-10 hashtags', 'engagement optimization'],
  },
  'Facebook Reels': {
    optimalDuration: { min: 15, max: 90, recommended: 30 },
    aspectRatio: '9:16',
    maxCaptionLength: 5000,
    hashtagCount: { min: 5, max: 10, recommended: 7 },
    contentType: ['community engagement', 'relatable content', 'shareable moments'],
    bestPractices: ['community engagement', 'relatable content', 'Facebook-specific hashtags', 'vertical format'],
  },
  'Twitter': {
    ...
    bestPractices: ['timely content', 'conversational tone', '1-2 hashtags', 'thread format for longer content'],
  },
  'LinkedIn': {
    ...
    bestPractices: ['professional tone', 'industry-specific hashtags', '3-5 hashtags', 'comments and shares'],
  },
};
    ...
    <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md text-sm">
      ...
      <div className="space-y-1 text-blue-800 dark:text-blue-300">
        {info.optimalDuration && (
          <p>
            ⏱️ Duration: {info.optimalDuration.recommended}s (optimal: {info.optimalDuration.min}s-{info.optimalDuration.max}s)
          </p>
        )}
        {info.aspectRatio && (
          <p>📐 Aspect Ratio: {info.aspectRatio}</p>
        )}
        {info.hashtagCount && (
          <p>
            #️⃣ Hashtags: {info.hashtagCount.recommended} recommended ({info.hashtagCount.min}-{info.hashtagCount.max})
          </p>
        )}
